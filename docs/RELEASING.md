...
version, creates checksums and attestations, renders deterministic notes from
the version section in `CHANGELOG.md`, uploads all eight assets to a draft,
publishes `labcoat-test` through crates.io OIDC, then publishes the GitHub
release as latest. Reruns are safe only when the existing tag points to the
same merge commit.

Each rendered release note contains user-facing changes, compatibility and
breaking-change guidance, version-pinned installation and attestation commands,
known limitations, and a link to the full changelog. A release section for the
exact workspace version is required; autogenerated GitHub notes are not used.
...
A CLI release is complete only after all of these checks pass against the
published artifacts:

- the installed `labcoat --help` exposes `new` and `build`;
- `labcoat deploy --help` accepts an exact Cargo package name, and a package
  dry run succeeds;
- `https://labcoat.sh/install` retrieves the published version when unpinned;
- every checksum verifies and GitHub attestations verify for all eight assets;
- the GitHub release body was rendered from the matching `CHANGELOG.md`
  section and documents compatibility and limitations;
- public stable-versus-main compatibility copy is updated only after the
  installer and bundled `labcoat docs --llm` reference are verified.

Do not move or replace `cli-v0.1.0`. Let the existing release-plz workflow
prepare the next CLI release, review and merge its release PR, verify the
installer and assets, and only then remove the temporary 0.1 compatibility
notice.
