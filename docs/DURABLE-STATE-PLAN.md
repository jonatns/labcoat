...
- normalized network;
- genesis hash;
- a Labcoat Network instance UUID for managed local instances;
- deployment transaction and block hashes;
- the last observed tip as a reorg anchor.

Every `labcoat reset` must generate a new Labcoat Network instance UUID. Remote regtest
...
- `chain_mismatch`: state belongs to a different chain or Labcoat Network instance.
...
- `isomer-core`: persistent Labcoat Network instance UUID regenerated by reset.
...
- Add Labcoat Network instance UUID generation and reset behavior.
...
- A reset Labcoat Network is rejected as a chain mismatch before mutation.
...
- **Reorgs and resets:** persist block hashes and Labcoat Network identity, refresh before
